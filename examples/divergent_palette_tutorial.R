...
## "value" type treemaps take divergent color palettes, so RdOrYl is considered as a divergent palette. To see what happens with divergent color palettes, I will use RdYlBu instead:
pal <- "RdYlBu"

## The center color (Yl) will automatically be mapped to 0, negative values to Rd-Yl and positive to Yl-Bu:
...
## By defautl a divergent color palette is mapped to

With the mapping argument, 
dat <- data.frame(letters=letters[1:26], x=1, y=runif(26)*16-12)
treemap(dat, index="letters", vSize="x", vColor="y", type="value", palette=pal)
