drawLegend <- function(labels, colors, bottom) {
	n <- length(labels)
	lineNpc <- convertHeight(unit(0.8,"lines"), "npc", valueOnly = TRUE)
	
	# display legend 
	if (bottom) {
		legX <- seq(0,1*((n-1)/n),length.out=n)
		legY <- rep(lineNpc,n)
		legW <- rep(1/n,n)
		legH <- rep(0.2,n)
		leg <- data.frame(X=legX,Y=legY,W=legW,H=legH,Col=colors)
		
		grid.rect(x=unit(leg$X,"npc"), y=unit(leg$Y,"npc"), width=unit(leg$W,"npc"), 
				  height=unit(leg$H,"npc"), just=c("left","bottom"), gp = gpar(fill = as.character(leg$Col)))
		grid.text(labels,x=unit(leg$X+.5*leg$W, "npc"),y=unit(0, "npc") + unit(0.5,"lines"),
				  gp=gpar(cex=0.8))	
	} else {
		legX <- rep(0.05, n)
		legY <- 1-1.5*lineNpc-((0:(n-1))*(lineNpc*1.5))
		legW <- rep(0.05, n)
		legH <- rep(lineNpc,n)
		leg <- data.frame(X=legX,Y=legY,W=legW,H=legH,Col=colors)
		
		grid.rect(x=unit(leg$X,"npc"), y=unit(leg$Y,"npc"), width=unit(leg$W,"npc"), 
				  height=unit(leg$H,"npc"), just=c("left","bottom"), gp = gpar(fill = as.character(leg$Col)))
		grid.text(labels,x=unit(leg$X+leg$W+0.02, "npc"),y=unit(leg$Y+.5*leg$H, "npc"),
				  gp=gpar(cex=1), just=c("left", "center"))	
	}
}